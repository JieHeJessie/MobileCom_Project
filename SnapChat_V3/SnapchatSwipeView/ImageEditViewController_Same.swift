...
// Almost same as ImageEidtViewController.swift. But due to solving bug for unwind, rename a new file.
class ImageEditViewController_Same: UIViewController, UITextFieldDelegate,UIPickerViewDelegate,UIPickerViewDataSource {
...
// --------------------------- Segue Preparation -------------------------------
    override func prepareForSegue(segue: UIStoryboardSegue, sender: AnyObject?) {
        ...
        }else if (segue.identifier == "goToSocialShare"){
            let destination = segue.destinationViewController as! socialShareViewController
            
            // Take a screenShot and send it to the destination
            destination.image = imageEditView.capture()
            destination.defaultContent = "Welcome to see my story"
        }
    }
    
    @IBAction func unwindToEditorInSnap(segue: UIStoryboardSegue) {}
